...
|---|-----------|--------|----------|----------|
| 1 | **체력측정 및 운동처방 종합 데이터** | `국민체력100_202506-202603/` (월별) | 약 201,845건 (청소년 35,522명) | 재능 진단 알고리즘 |
...
- OpenAI(생성형 AI, gpt-4o-mini)를 활용한 존중 기반 맞춤형 코칭 코멘트 생성
...
| 데이터명 | 파일명 | 활용 목적 |
|---------|--------|----------|
| 체력측정 및 운동처방 종합 데이터 | `국민체력100_202506-202603/` (월별, 2025.6~2026.3) | 연령별 체력 백분위 산출, 재능 스코어링 기초 지표 |
...
| 스포츠강좌이용권 현황 | `지역별스포츠강좌이용권활용정보(202507).csv` | 재능–지원 격차 분석 |
...
4. OpenAI(gpt-4o-mini) → AI 기반 맞춤 코멘트 생성
...
| 등급 | 기준 | 백분위 |
|------|------|--------|
| 최우수 (excellent) | 82점 이상 | 상위 5% |
| 우수 (high) | 69점 이상 | 상위 15% |
| 평균이상 (above_average) | 54점 이상 | 상위 35% |
| 평균 (average) | 43점 이상 | 상위 55% |
| 평균이하 (below_average) | 43점 미만 | 하위 45% |

### 5.4 AI 코멘트 생성
- OpenAI(gpt-4o-mini)를 통해 재능 결과에 대한 자연어 설명과 훈련 방향 코멘트 자동 생성 (존중·정직 가드레일 내장)
...
└── code/
    ...
    └── backend/                       # 백엔드 (FastAPI)
        ├── data/                      # 공공데이터 CSV 파일
        │   ├── 국민체력100_202506-202603/  # 체력측정 월별 데이터 (2025.6~2026.3)
        ...
        │   │   └── llm_client.py     # OpenAI(생성형 AI) 연동
        │   └── scripts/              # ETL 스크립트
